...
- **[P2] OpenOCD backend**: TCL 关键词匹配 + 进程级重启（超时杀子进程→重启→重新 attach）+ Docker CI 固化 0.12 版本。（✅ 全功能 Debug 后端已完成：halt/step/resume/断点/内存/寄存器/watchpoint。`try_recover` 保留 P2 待实现）
...
- **[2026-06-07]**: 归档 OpenOCD 全功能 Debug 后端需求。量子交叉审计 4 道红线全部守牢，104/104 测试通过。提炼 1 条 TCL 响应解析经验反哺 AGENTS.md（read_memory Tcl list 花括号解析、reg 命令 (dirty) 标记处理、wait_halt 空响应 vs halt/step 错误关键词两种模式）。目录已归档至 [user_plan/archive/openocd-debug-backend/](user_plan/archive/openocd-debug-backend/)。(By Agent - Archive-and-Summary)
- **[2026-06-07]**: 实现 OpenOCD 全功能 Debug 后端（P2）。通过 Understanding (Grill Me) 确认 Q1=A（debug 子命令新增 `--openocd-cfg` 参数）、Q2=A（resume 语义通过 target_halted 标志区分）。实现 11 个 formerly-stubbed 方法（halt/step/resume/set_breakpoint/clear_breakpoint/read_mem/write_mem/read_regs/is_halted/poll_halted + watchpoint），复用 TCL telnet 通道。`create_debug_backend()` 改造支持 `--openocd-cfg` CLI > `.debugger/openocd.cfg` 兜底。全量回归：cargo fmt + cargo check 零 warning + 104/104 测试通过。需求文档已生成于 [user_plan/openocd-debug-backend/openocd-debug-backend.md]。(By Agent - Implementation)
